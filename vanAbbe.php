  ...
  return 2.2 * n_xyz;
		...
		void main() {
			...
			vUv = uv;
				...
				noise = 3.0 *  -.3 * turbulence( 0.9 * normal + time);
				float b = 1.0 * pnoise( 0.015 * position + vec3( 2.0 * time ), vec3( 100.0 ) ); // SMALL NOISES 

				float smallNoise = 3.0 *  -.3 * turbulence( .5 * normal);
				float smallB =  1.0 * pnoise( .9 * position + vec3( 600.0 ), vec3( 100.0 ) );

				float bigNoise = 3.0 *  -.3 * turbulence( 0.9 * normal + time);
				float bigB = 1.0 * pnoise( 0.8 * position + vec3( 2.0 * time ), vec3( 100.0 ) );

				float displacement = - noise + b *15.0;
				displacement +=  - smallNoise + smallB *distortion;
			//	displacement *= cos(-bigNoise + bigB)*20.0;
	...
	<script type="x-shader/x-fragment" id="fragmentshaderrr">
		varying vec2 vUv;
		varying float noise;
